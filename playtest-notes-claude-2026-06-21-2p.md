...
- **The AI reflexively shadows the human's bids.** On materials its hand wanted, the Mink bid into *my* auctions repeatedly, and on small cards that guaranteed mutual jams (it cost me a reed, a log, and several wasted fish, and cost itself fish too). Reading `aiDecideBid`, the AI bids to its hand-need with no awareness that on a tiny card a contesting bid just torches both players. A human opponent would learn to stop; the AI doesn't, which makes the late game grindier than it should be.
