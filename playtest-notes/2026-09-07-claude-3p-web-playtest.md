...
2. **Burrow Run's build logged as "(0🐟, 4★)" but the card's printed fish cost is 2.** I paid nothing visible and then slid back 5. If some discount applied (nothing in my tableau discounts vine/mud builds), I can't identify it; either the fish cost wasn't charged, or the log line under-reports what was charged before the −5 slide. Worth a look at the build-cost path for when-built movers. *(Observed once; cross-check `sim.js` build cost vs the web log line.)*
